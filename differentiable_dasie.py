...
def main(flags):
    ...
    for aperture_num in range(num_apertures):
        ...
        pupil_plane += aperture_function_2d(X, Y, mu_u, mu_v, alpha, beta,
                                            tip_phase, tilt_phase, pison_phase)
    ...
    psf = np.abs(
        np.fft.fftshift(np.fft.fft2(np.fft.ifftshift(pupil_plane)))) ** 2
    ...
    distributed_aperture_image = np.abs(
        (np.fft.fft2(distributed_aperture_image_spectrum)))
    distributed_aperture_image = distributed_aperture_image / np.max(
        distributed_aperture_image)
    distributed_aperture_image_cosine_similarity = cosine_similarity(
        distributed_aperture_image, perfect_image_flipped)
    print("Cosine similarity: " + str(
        distributed_aperture_image_cosine_similarity))
