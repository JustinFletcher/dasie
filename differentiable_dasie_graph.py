...
def zernike_5(T):
    ...
    cartesian_azimuth = tf.math.atan2(v_field, u_field)
    # Should be rho ** 2
    z = tf.math.sqrt(tf.constant(6.0, dtype=tf.float64)) * (cartesian_radial ** 3) * tf.math.cos(2 * cartesian_azimuth)
    z = tf.cast(z, dtype=tf.complex128)
...
def zernike_6(T):
    ...
    cartesian_radial = tf.math.sqrt(u_field**2 + v_field**2)
    cartesian_azimuth = tf.math.atan2(v_field, u_field)
    z = tf.math.sqrt(tf.constant(8.0, dtype=tf.float64)) * tf.math.pow(cartesian_radial, tf.constant(3.0, dtype=tf.float64)) * tf.math.sin(3.0 * cartesian_azimuth)
    z = tf.cast(z, dtype=tf.complex128)
